...
stop_unsuccessful = True
# stop_unsuccessful = False
...
# cec=4
cec=float(args['cec'])
targetpH = float(args['tph'])
acidsat = float(args['tec'])
targetOM = float(args['tsom'])
# targetlogpco2 = -2.
targetlogpco2 = float(args['tsoilco2'])

poro_input = float(args['poro'])
moist_input = float(args['soilmoisture'])
sat_input = float(args['soilmoisture'])/float(args['poro'])
...
tau_g2 = 1. / ( 1./8.*3.**( (float(args['mat']) - 15.)/10.  ) )
# print(tau_g2)

runname_guess = args['initguess']
...
zdust_lab=0.15
w_field=100e-5
w_field=float(args['erosion'])
w_lab=0
q_field=1200e-3
q_field=float(args['qrun'])
q_lab=0
p=10e-6
nstep=10
rstrt='self'
...
N_rain = float(args['nitrif'])   # gN/m2/yr ( <---> 220 lbs/acre/year)
...
def run_a_single_set(ca,logkh,omrain_field,tau_g2):
    ...
    N_rain = float(args['nitrif'])  # gN/m2/yr
    N_rain = N_rain/14  # mol N/m2/yr
    N_rain = N_rain*80  # g NH4NO3/m2/yr
    N_rain = N_rain/2.  # only half is required as 1 mol NH4NO3 contains 2 moles of N
    N_rain = N_rain/(omrain_field)  # normalize against OC rain 
    ...
    my_timeout =60*60  # kill the run if not complete in this many seconds
