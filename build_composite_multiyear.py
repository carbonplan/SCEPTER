...
def find_subdirs_1level(root_dir, target_string, field_only=False):
    ...
    if field_only:
        ...
        filtered_directories = [directory for directory in matching_subdirectories if "lab" not in directory]
    ...
    for src_tmp in alldirs:
        ...
        for fn in flx_fns_tmp:
            ...
            if filecheck: # then exclude the header, append the rest
                dfsrc.to_csv(savedst_tmp, header=None, index=None, sep='\t', mode="a")   # mode = "a" will append to end of file if exists
            else:         # if the file doesn't exist, save and include the header
                dfsrc.to_csv(savedst_tmp, index=None, sep='\t', mode="a")   # mode = "a" will append to end of file if exists
        # ----------------------------------------------------------
